...
}@${env.mongoose.host}:${env.mongoose.port || "27017"}/${
  env.mongoose.database
}?tls=true&replicaSet=rs0&readPreference=secondaryPreferred&retryWrites=false`;
const ca = `${path.join(
  __dirname,
  `../../ssl/${env.mongoose.cert || "rds-combined-ca-bundle.pem"}`
)}`;
...
if (env.db_type === "mongodb_aws") {
  ...
  mongoConn = mongoose
    .connect(
      `mongodb://${env.mongoose.host}:${env.mongoose.port}/${env.mongoose.database}`,
      {
